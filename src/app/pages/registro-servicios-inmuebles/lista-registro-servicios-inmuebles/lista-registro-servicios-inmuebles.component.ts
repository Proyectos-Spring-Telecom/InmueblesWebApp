...
} from '../../monitoreo/monitoreo-inmueble.mapper';
import {
  construirFormDataPagoInmueble,
...
@Component({
  selector: 'app-lista-registro-servicios-inmuebles',
  templateUrl: './lista-registro-servicios-inmuebles.component.html',
  styleUrl: './lista-registro-servicios-inmuebles.component.scss',
  standalone: false,
  animations: [routeAnimation, contractDimAnim, contractModalAnim],
})
export class ListaRegistroServiciosInmueblesComponent implements OnInit {
  ...
  pagoForm!: FormGroup;
  pagoMontoRaw = '';
  serviciosInmuebleModalPago: ServicioInmueblePagoOpcion[] = [];
  serviciosInmuebleModalPagoCargando = false;
  ...
  ngOnInit(): void {
    ...
    this.cargarCatalogoInmuebles();
    this.setupDataSource();
  }

  private initPagoForm(): void {
    this.pagoForm = this.fb.group({
      idInmueble: [null, Validators.required],
    ...
    this.pagoForm.get('idInmueble')?.valueChanges.subscribe((id) => {
      this.onInmuebleModalChange(id);
    });
  }
  ...
  private cargarCatalogoInmuebles(): void {
    this.catalogosFiltroCargando = true;
    void lastValueFrom(this.inmueblesService.obtenerInmueblesData(1, 300))
      .then((resp) => {
        const rows = Array.isArray(resp?.data) ? resp.data : [];
        this.inmueblesOpciones = mapInmueblesApiToGridRows(rows).map((r) => ({
          id: r.id,
          label: r.inmueble || `Inmueble #${r.id}`,
        }));
      })
      .catch((err) => console.error('Error catálogo inmuebles:', err))
      .finally(() => {
      ...
      load: async (loadOptions: { take?: number; skip?: number }) => {
        ...
        try {
          const resp = await lastValueFrom(
            this.pagoInmuebleService.obtenerPagosPaginados({
              page,
              limit: take,
          ...
          this.loading = false;
          console.error('Error al cargar pagos de servicios del inmueble:', err);
  ...
  verComprobanteDetalle(): void {
    ...
    this.docPreview?.abrir(previewUrl, titulo, 'Servicios inmueble');
  }
  ...
  abrirModalPago(): void {
    ...
    this.pagoMontoRaw = '';
    this.serviciosInmuebleModalPago = [];
    const idInmuebleInicial = this.idInmuebleFiltro;
    this.pagoForm.reset({
      idInmueble: idInmuebleInicial,
      idServicioInmueble: null,
      concepto: '',
      fechaPago: '',
      monto: '',
    ...
    if (idInmuebleInicial != null) {
      this.cargarServiciosParaModalPago(idInmuebleInicial);
    }
    this.cdr.markForCheck();
    setTimeout(() => {
  ...
  private onInmuebleModalChange(idInmueble: unknown): void {
    const id = Number(idInmueble);
    this.pagoForm.patchValue(
      { idServicioInmueble: null, concepto: '' },
      { emitEvent: false },
    );
  ...
  private cargarServiciosParaModalPago(idInmueble: number): void {
    this.serviciosInmuebleModalPago = [];
    this.serviciosInmuebleModalPagoCargando = true;
    this.inmueblesService
      .obtenerInmueble(idInmueble)
      .pipe(
        take(1),
        catchError((err) => {
          console.error('Error al cargar servicios del inmueble:', err);
          void Swal.fire({
            background: '#141a21',
            color: '#ffffff',
            icon: 'error',
            title: 'No se pudieron cargar los servicios',
            text: 'No se obtuvo la lista de servicios del inmueble. Intenta de nuevo.',
            confirmButtonText: 'Entendido',
          });
          return of(null);
        }),
        finalize(() => {
          this.serviciosInmuebleModalPagoCargando = false;
          this.cdr.markForCheck();
        }),
      )
      .subscribe((resp) => {
        const item = extraerInmuebleDetalleApi(resp);
        this.serviciosInmuebleModalPago = buildServiciosMonitoreoInmueble(item)
          .filter((s) => s.idServicioInmueble != null)
          .filter((s) => !esServicioArrendatarioExcluidoHub(s.concepto))
          .map((s) => ({
            id: s.idServicioInmueble as number,
            nombre: s.concepto,
          }));
      });
  }
  ...
  onPagoServicioInmuebleChange(): void {
    ...
    const hit = this.serviciosInmuebleModalPago.find((s) => s.id === numId);
    const etiqueta = String(hit?.nombre ?? '').trim();
    if (!etiqueta) return;
  ...
  guardarPagoDesdeModal(): void {
    ...
    if (this.pagoForm.invalid) {
      void Swal.fire({
        ...
        icon: 'warning',
        title: 'Faltan datos',
        text: 'Completa inmueble, fecha, monto y comprobante.',
        confirmButtonText: 'Entendido',
      });
      return;
    }
    ...
    const idInmueble = Number(v.idInmueble);
    ...
    const fd = construirFormDataPagoInmueble(
      Math.floor(idInmueble),
      v,
      montoN,
      archivo,
    );

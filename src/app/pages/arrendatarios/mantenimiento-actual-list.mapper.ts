...
export interface MantenimientoActualGridRow {
  id: number;
...
export function extraerFilasMantenimientoActualApi(resp: unknown): unknown[] {
...
function etiquetaPeriodoMes(row: Record<string, unknown>): string {
  const raw = row['mes'] ?? row['mesMantenimiento'] ?? row['periodo'];
...
export function mapMantenimientoActualApiToGridRow(
  item: unknown,
): MantenimientoActualGridRow | null {
  const row = item as Record<string, unknown>;
  const id = num(row['id'] ?? row['idMantenimientoActual']);
  ...
  return {
    ...
    factorVariable,
    factorVariableFmt: factorVariable != null ? String(factorVariable) : '—',
    ocupoFormula,
...
export function extraerMantenimientoActualDetalleApi(
  resp: unknown,
  ...
  if (typeof resp === 'object' && !Array.isArray(resp)) {
    ...
    if (r['id'] != null || r['idMantenimientoActual'] != null) return r;
  }
  return null;
}

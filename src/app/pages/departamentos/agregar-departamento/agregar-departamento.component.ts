...
@Component({
  ...
  styleUrl: './agregar-departamento.component.scss',
  standalone: false,
...
export class AgregarDepartamentoComponent {
  ...
  public title = 'Agregar Departamento';
  ...
  obtenerDepartamento() {
    this.departamentosService.obtenerDepartamento(this.idDepartamento).subscribe((res: any) => {
  ...
  agregar() {
    ...
    if (this.departamentoForm.invalid) {
      ...
      Object.keys(this.departamentoForm.controls).forEach((key) => {
        const control = this.departamentoForm.get(key);
    ...
    this.departamentosService.agregarDepartamento(payload).subscribe(
      (response: any) => {
        this.submitButton = 'Guardar';
        this.loading = false;
        Swal.fire({
          title: '¡Operación Exitosa!',
          text: `Se agregó un nuevo departamento de manera exitosa.`,
          icon: 'success',
  ...
  actualizar() {
    ...
    this.departamentosService.actualizarDepartamento(this.idDepartamento, payload).subscribe(
      (response: any) => {
        this.submitButton = 'Actualizar';
        this.loading = false;
        Swal.fire({
          title: '¡Operación Exitosa!',
          text: `Los datos del departamento se actualizaron correctamente.`,
          icon: 'success',
          confirmButtonColor: '#3085d6',
          confirmButtonText: 'Confirmar',
          background: '#141a21',

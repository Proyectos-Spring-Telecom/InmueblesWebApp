...
@Component({
  selector: 'app-lista-clientes-inmuebles',
  templateUrl: './lista-clientes-inmuebles.component.html',
  styleUrl: './lista-clientes-inmuebles.component.scss',
  standalone: false,
  animations: [routeAnimation],
})
export class ListaClientesInmueblesComponent implements OnInit {
  ...
  constructor(
    private cliService: ClientesInmueblesService,
  ...
  agregarCliente() {
    this.route.navigateByUrl('/clientes-inmuebles/agregar-cliente-inmueble');
  }
        ...
        try {
          ...
          const dataTransformada = (
            ...
            .map((item: any) => {
              ...
              const paterno = String(item?.apellidoPaterno ?? '').trim();
              const materno = String(item?.apellidoMaterno ?? '').trim();
              const nombreCompleto = [nombre, paterno, materno]
                .filter(Boolean)
                .join(' ');

              const partesDir = [
                item?.calle ? `Calle ${String(item.calle).trim()}` : '',
                item?.numeroExterior ? `#${String(item.numeroExterior).trim()}` : '',
                item?.numeroInterior ? `Int. ${String(item.numeroInterior).trim()}` : '',
                String(item?.colonia ?? '').trim(),
                String(item?.municipio ?? '').trim(),
                String(item?.estado ?? '').trim(),
                item?.cp ? `CP ${String(item.cp).trim()}` : '',
                item?.entreCalles
                  ? `(Entre calles: ${String(item.entreCalles).trim()})`
                  : '',
              ].filter(Boolean);
              const direccionCompleta = partesDir.join(', ');

              const tipoRaw = Number(item?.tipoPersona);
              const tipoPersona =
                tipoRaw === 1 ? 'Físico' : tipoRaw === 2 ? 'Moral' : '—';
  ...
  actualizarCliente(idCliente: number) {
    this.route.navigateByUrl('/clientes-inmuebles/editar-cliente-inmueble/' + idCliente);
  }
